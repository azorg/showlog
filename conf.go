...
type Conf struct {
	URI    string  `json:"uri,omitempty"`    // log file
...
func (c *Conf) Read() {
	...
	log.Printf("read conf: URI='%s' Center=%v Full=%v W=%.1f H=%.1f",
		C.URI, C.Center, C.Full, C.W, C.H)
...
func (c *Conf) Write() {
	...
	log.Printf("write conf: URI='%s' Center=%v Full=%v W=%.1f H=%.1f",
		c.URI, c.Center, c.Full, c.W, c.H)
}
